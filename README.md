...
When placed into `/Library/LaunchDaemons`, these files are loaded at system startup for every user. When placed in `/Library/LaunchAgents`, they load at login for every user. When placed in `~/Library/LaunchAgents` they load at login for that particular user.
